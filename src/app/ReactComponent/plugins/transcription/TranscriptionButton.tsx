...
const pusher: any =
  ...
  new Pusher(enviornmentConstant.REACT_APP_TRANSCRIPTION_PUSHER_APP_KEY, {
    cluster: "ap2",
    authEndpoint:
      enviornmentConstant.REACT_APP_TRANSCRIPTION_PUSHER_AUTHENDPOINT,
  });
...
export function TranscriptionButton() {
  ...
  const transcriber: any = useRef(null);
  ...
  useEffect(() => {
    ...
    channel.bind(`client-transcription`, ({ text }: any) => {
      if (text) {
...
class Transcriber {
  ...
  initialized = false;
  lastMessage: any = {};
  localPeerId: any = null;
  sttTuningConfig: any;
  constructor(setTranscript: any, setSpeakingPeer: any) {
    this.enabled = false;
    this.socket = null;
  ...
  broadcast = (text: any, eventName = "transcription") => {
  ...
  async listen() {
    try {
      ...
      let url: any =
        enviornmentConstant.REACT_APP_DYNAMIC_STT_TOKEN_GENERATION_ENDPOINT;
  ...
  async observeStream(stream: any) {
  ...
  enableTranscription(enable: any) {
