 ...
 */
export function useMultiplayerState(roomId: any) {
  ...
  const updateLocalState = useCallback(({ shapes, bindings, merge }: any) => {
    if (!(shapes && bindings)) return;

    if (merge) {
      const lShapes: any = rLiveShapes.current;
      const lBindings: any = rLiveBindings.current;

      if (!(lShapes && lBindings)) return;
      Object.entries(shapes).forEach(([id, shape]: any) => {
        if (!shape) {
          lShapes.delete(id);
        } else {
      ...
      Object.entries(bindings).forEach(([id, binding]: any) => {
        if (!binding) {
          lBindings.delete(id);
        } else {
  ...
  const onChangePage = useCallback(
    (_app: any, shapes: any, bindings: any, _assets: any) => {
