...
export const StatsForNerds = ({ onOpenChange }: any) => {
  ...
  const [showStatsOnTiles, setShowStatsOnTiles]: any = useSetUiSettings(
...
const useTracksWithLabel = () => {
  ...
  const tracksWithLabels = useMemo(
    () =>
      Object.values(tracksMap).map((track: any) => {
...
const StatsRow = ({ label, value }: any) => (
...
const formatBytes = (bytes: any, unit = "B", decimals = 2) => {
