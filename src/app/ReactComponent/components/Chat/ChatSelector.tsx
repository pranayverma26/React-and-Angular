...
const SelectorItem = ({ value, active, onClick, unreadCount }: any) => {
  return (
    <Dropdown.Item
...
const SelectorHeader = React.memo(({ children }: any) => {
  return (
...
const Everyone = React.memo(({ onSelect, active }: any) => {
...
const RoleItem = React.memo(({ onSelect, role, active }: any) => {
...
const PeerItem = ({ onSelect, peerId, name, active }: any) => {
...
const VirtualizedSelectItemList = ({
  ...
  const listItems = useMemo(() => {
    ...
    filteredPeers.forEach((peer: any) =>
      selectItems.push(
        <PeerItem
          key={peer.id}
          name={peer.name}
  ...
  return (
    <Dropdown.Group css={{ height: "$64", overflowY: "auto" }}>
      <FixedSizeList
        itemSize={52}
...
export const ChatSelector = ({ role, peerId, onSelect }: any) => {
