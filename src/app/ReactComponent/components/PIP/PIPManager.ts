...
class PipManager {
   ...
   */
  async start(hmsActions: any, onStateChangeFn: any) {
    if (!this.isSupported()) {
   ...
   * */
  async updatePeersAndTracks(peers: any, tracksMap: any) {
    if (!this.canvas) {
      return;
    }
   ...
   * @private
   */
  async init(hmsActions: any, onStateChangeFn: any) {
   ...
   */
  pickTracksToShow(peers: any, tracksMap: any) {
    const tracksToShow = [];
   ...
   * @return {Array}
   */
  orderNewTracksToShow(newTracks: any, oldTracks: any) {
