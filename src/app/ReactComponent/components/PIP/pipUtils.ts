 ...
 */
export function drawVideoElementsOnCanvas(videoElements: any, canvas: any) {
  let numberOfTiles = videoElements.filter(
    (videoElement: any) => videoElement.srcObject !== null
  ).length;
...
export function dummyChangeInCanvas(canvas: any) {
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#000000';
  const canvasWidth = canvas.width;
  const canvasHeight = canvas.height;
