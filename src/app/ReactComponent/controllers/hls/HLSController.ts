...
export class HLSController {
  ...
  isLive = true;
  ControllerEvents: any;
  constructor(hlsUrl: any, videoRef: any) {
  ...
  setCurrentLevel(currentLevel: any) {
    const newLevel = this.hls.levels.findIndex(
      (level: any) => level.height === currentLevel.height
    );
  ...
  enableTimeUpdateListener() {
    this.videoRef.current.addEventListener('timeupdate', (_: any) => {
      if (this.hls) {
        const videoEl = this.videoRef.current;
        const allowedDelay =
  ...
  handleHLSTimedMetadataParsing() {
     ...
     */
    this.hls.on(Hls.Events.FRAG_CHANGED, (_: any, { frag }: any) => {
      try {
